...
const addToCart = (index) => {
  cy.get(ITEM_SELECTOR).should('exist').eq(index).children(ADD_CART_BUTTON).click();
};
...
const checkPrice = (results, indexes) => {
  ...
  cy.get(TOTAL_PRICE).should('have.text', total.toString());
};
...
describe('Shopping Cart Project', () => {
  ...
  describe('1 - Crie uma listagem de produtos', () => {
    it('Listagem de produtos', () => {
      cy.get(ITEM_SELECTOR).should('exist').should('have.length', results.length);
    });
  });
  ...
  describe('2 - Adicione o produto ao carrinho de compras', () => {
    it('Adicione o produto ao carrinho de compras', () => {
      ...
      cy.get(CART_ITEMS)
        ...
        .should(
          'have.text',
          `SKU: ${results[36].id} | NAME: ${results[36].title} | PRICE: $${results[36].price}`,
        );
    });
  });
  ...
  describe('4 - Carregue o carrinho de compras através do **LocalStorage** ao iniciar a página', () => {
    it('Carregue o carrinho de compras através do **LocalStorage** ao iniciar a página', () => {
      ...
      cy.get(CART_ITEMS)
        ...
        .should(
          'have.text',
          `SKU: ${results[first].id} | NAME: ${results[first].title} | PRICE: $${results[first].price}`,
        );
      ...
      cy.get(CART_ITEMS)
        ...
        .should(
          'have.text',
          `SKU: ${results[last].id} | NAME: ${results[last].title} | PRICE: $${results[last].price}`,
        );
  ...
  describe('8 - Desenvolva testes para atingir 25% de cobertura total e 100% da função fetchProducts', () => {
    it('Verifica a cobertura de testes unitários', () => {
      cy.exec(
        'npm run test:coverage -- --coverageReporters="json-summary" --testFailureExitCode=0 && npm run serialize',
      );
      cy.readFile('coverage/coverage-summary.json')
        .its('total.functions.pct')
        .should('be.gte', 25.0);
      cy.readFile('coverage/coverage-summary.json')
        .its('fetchProducts.functions.pct')
        .should('be.gte', 100.0);
      cy.readFile('coverage/coverage-summary.json')
        .its('fetchProducts.lines.pct')
        .should('be.gte', 100.0);
    });
  });
  ...
  describe('9 - Desenvolva testes para atingir 50% de cobertura total e 100% da função fetchItem', () => {
    it('Verifica a cobertura de testes unitários', () => {
      ...
      cy.readFile('coverage/coverage-summary.json')
        ...
        .should('be.gte', 50.0);
      cy.readFile('coverage/coverage-summary.json')
        .its('fetchItem.functions.pct')
        .should('be.gte', 100.0);
      cy.readFile('coverage/coverage-summary.json')
        .its('fetchItem.lines.pct')
        .should('be.gte', 100.0);
    });
  });
